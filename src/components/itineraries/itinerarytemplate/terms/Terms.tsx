// src/components/itinerarytemplate/terms/Terms.tsx
...
export default function Terms({ customTerms }: TermsProps) {
  const renderCustomTerms = () => {
    // Split the terms into sections based on numbered items
    const sections = customTerms?.split(/(?=\d+\.)/).filter(Boolean) || [];
    ...
    return sections.map((section, index) => {
      ...
      return (
        <div key={index} className={styles.termsSection}>
          <h3 className={styles.sectionTitle}>{title.trim()}</h3>
